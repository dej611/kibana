...
export const AxisSettingsPopover: React.FunctionComponent<AxisSettingsPopoverProps> = ({
  ...
  const onExtentChange = useCallback(
    (newExtent) => {
      ...
      if (setExtent && newExtent && !boundaryError && !isEqual(newExtent, extent)) {
        if (axis === 'x' || !inclusiveZeroError) {
          setExtent(newExtent);
        }
      }
    },
    ...
    <>
      ...
      {extent?.mode === 'custom' && !hasPercentageAxis && (
        <RangeInputField
          ...
          testSubjLayout="lnsXY_axisExtent_customBounds"
          testSubjLower="lnsXY_axisExtent_lowerBound"
          testSubjUpper="lnsXY_axisExtent_upperBound"
